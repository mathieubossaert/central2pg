/*
FUNCTION: get_form_datasets_from_central(text, text, text, integer, text)
	description :
		Returns the lists of "entity lists" related to the given form.
	...
	returning :
		TABLE(user_name text, pass_word text, central_fqdn text, project integer, form text, dataset text)
*/

CREATE OR REPLACE FUNCTION odk_central.get_form_datasets_from_central(
	email text,
	password text,
	central_domain text,
	project_id integer,
	form_id text)
    RETURNS TABLE(user_name text, pass_word text, central_fqdn text, project integer, form text, dataset text) 
    LANGUAGE 'plpgsql'
    COST 100
...
BEGIN
url = replace(concat('https://',central_domain,'/v1/projects/',project_id,'/forms/',form_id,'/dataset-diff'),' ','%%20');
...
FORMAT('WITH data AS (SELECT json_array_elements(form_data) AS dataset FROM central_json_from_central)
...
ALTER FUNCTION odk_central.get_form_datasets_from_central(text, text, text, integer, text)
    OWNER TO dba;

COMMENT ON FUNCTION odk_central.get_form_datasets_from_central(text, text, text, integer, text)
    IS 'description :
		Returns the lists of "entity lists" related to the given form.
