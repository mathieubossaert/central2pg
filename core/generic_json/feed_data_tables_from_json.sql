CREATE OR REPLACE FUNCTION odk_central.feed_data_tables_from_json(
	schema_name text,
	table_name text,
	unique_column text,
...
EXECUTE format('SET search_path=odk_central,pubic;
	...
	WITH RECURSIVE doc_key_and_value_recursive(data_id, key, value) AS (
	  SELECT
		('||table_name||'.form_data ->> '''||unique_column||''') AS data_id,
		t.key,
		t.value
...
RAISE INFO 'exiting from feed_data_tables_from_json for table %', table_name; 
