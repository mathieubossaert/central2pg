...
/*
FUNCTION: get_dataset_from_central(text, text, text, integer, text, text, text)
	description
	...
	parameters :
		email text						-- the login (email adress) of a user who can get datasets
		...
		dataset text					-- the name of the dataset to download
...
CREATE OR REPLACE FUNCTION odk_central.get_dataset_from_central(
	email text,						
	password text,					
	central_domain text, 			
	project_id integer,				
...
BEGIN
url = replace(concat('https://',central_domain,'/v1/projects/',project_id,'/datasets/',dataset,'.svc/Entities'),' ','%%20');
EXECUTE (
		'DROP TABLE IF EXISTS central_json_from_central;
		 CREATE TABLE central_json_from_central(form_data json);'
		);
...
COMMENT ON FUNCTION  odk_central.get_dataset_from_central(text,text,text,integer,text,text,text)
	IS 'description :
		Get json data from Central, feed a temporary table with a generic name central_json_from_central.
